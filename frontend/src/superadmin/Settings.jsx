...
function Settings({ onUpdate }) {
    ...
    const checkAccess = async (employeeID) => {
        try {
            const response = await axios.get(`http://192.168.50.2:5000/api/page_access/${employeeID}/${pageId}`);
    ...
    useEffect(() => {
        axios.get(`http://192.168.50.2:5000/api/settings`)
            .then(({ data }) => {
                setCompanyName(data.company_name || "");
                setShortTerm(data.short_term || "");
                setAddress(data.address || "");
                setPreviewLogo(data.logo_url ? `http://192.168.50.2:5000${data.logo_url}` : null);
                setPreviewBg(data.bg_image ? `http://192.168.50.2:5000${data.bg_image}` : null);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post(`http://192.168.50.2:5000/api/settings`, formData, { headers: { "Content-Type": "multipart/form-data" } });
            onUpdate?.();
