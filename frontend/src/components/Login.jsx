...
const Login = ({ setIsAuthenticated }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const apiUrl =
        loginType === "applicant"
          ? `http://192.168.50.2:5000/login_applicant`
          : `http://192.168.50.2:5000/login`;
  ...
  const backgroundImage = settings?.bg_image
    ? `url(http://192.168.50.2:5000${settings.bg_image})`
    : "linear-gradient(to right, #f5f5f5, #fafafa)";
  const logoSrc = settings?.logo_url
    ? `http://192.168.50.2:5000${settings.logo_url}`
    : Logo;
